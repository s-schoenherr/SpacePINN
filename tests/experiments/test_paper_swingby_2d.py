...
def test_build_configs_sets_paper_labels_and_smoke_without_mutating_presets():
    module = importlib.import_module("spacepinn.paper.swingby_2d")
    ...
    assert configs[0]["plotting"]["quiver_count"] == module.QUIVER_COUNT
    assert configs[1]["plotting"]["linestyle"] == "solid"
    assert configs[1]["plotting"]["trajectory_linestyle"] == "solid"
    assert configs[1]["plotting"]["quiver_count"] == module.QUIVER_COUNT
...
def test_main_configures_paper_plotter_styling(monkeypatch):
    ...
    assert init_calls[0]["labels"] == [module.GEOMETRIC_LABEL, module.ORDINARY_LABEL, module.BASELINE_LABEL]
    assert init_calls[0]["figsize"] == module.MAIN_FIGSIZE
    assert init_calls[1]["labels"] == [module.GEOMETRIC_LABEL, module.ORDINARY_LABEL]
    assert init_calls[1]["figsize"] == module.LOSS_FIGSIZE
    assert ("traj2d", module.MAIN_LINEWIDTH, module.SECONDARY_LINEWIDTH) in events
    assert ("loss", module.MAIN_LINEWIDTH, module.SECONDARY_LINEWIDTH) in events
