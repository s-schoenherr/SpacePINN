...
class Spaceship:
    ...
    def compute_gravitiy_polar(self, r, theta):
        del theta
...
def _cold_start_guess(
    obj: Spaceship,
    time_all_section: np.ndarray,
    *,
    alpha_final_guess: float | None = None,
) -> dict[str, np.ndarray]:
    alpha_guess = float(obj.xf[-1] if alpha_final_guess is None else alpha_final_guess)
    time_span = float(time_all_section[-1] - time_all_section[0] + 1e-15)
    # Keep the signed-control formulation, but mimic the OpenGoddard example's
    # "positive-thrust" bias with a small tangential acceleration seed.
    tangential_thrust_guess = max(1e-6, abs(float(obj.vf[-1] - obj.v0[-1])) / time_span)
    return {
        "r": Guess.linear(time_all_section, obj.x0[0], obj.xf[0]),
        "alpha": Guess.linear(time_all_section, obj.x0[-1], alpha_guess),
        ...
        "ut": Guess.constant(time_all_section, min(tangential_thrust_guess, obj.u_max)),
    }
...
def _solver_payload(
    *,
    ...
    return {
        "backend": "OpenGoddard",
        "problem": "circular_orbit_transfer_polar",
        "variant": label,
        "solver": {
            "time_init": time_init,
            "n": n,
...
def _orbit_transfer_goddard(
    *,
    label: str,
    enforce_velocity_constraints: bool,
    enforce_terminal_angle: bool = True,
    warm_start_result=None,
    max_iteration: int = 5,
    ftol: float = 1e-12,
    ...
    def dynamics(prob: Problem, obj: Spaceship, section):
        ...
        alpha = prob.states(3, section)
        ur = prob.controls(0, section)
        ut = prob.controls(1, section)

        gravity_rho = obj.compute_gravitiy_polar(r, alpha)
    ...
    def equality(prob: Problem, obj: Spaceship):
        ...
        alpha = prob.states_all_section(3)

        result = Condition()
        result.equal(r[0], obj.x0[0])
        result.equal(r[-1], obj.xf[0])
        result.equal(alpha[0], obj.x0[-1])
        if enforce_terminal_angle:
            result.equal(alpha[-1], obj.xf[-1])

        if enforce_velocity_constraints:
    ...
    prob.set_states_all_section(3, guess["alpha"])
    ...
    def display_func():
        ...
        alpha_final = prob.states_all_section(3)[-1]
        print(f"tf: {tf:.5f}")
        print(f"alpha_final: {alpha_final:.5f}")
...
def kinematic_ot_goddard_free_final_angle(
    label="Direct collocation",
    *,
    warm_start_result=None,
    max_iteration: int = 5,
...
def geometric_ot_goddard(
    ...
    *,
    ...
    return _orbit_transfer_goddard(
        label=label,
        enforce_velocity_constraints=False,
        enforce_terminal_angle=True,
        warm_start_result=warm_start_result,
        max_iteration=max_iteration,
        ftol=ftol,
...
if __name__ == "__main__":
    ...
    result = kinematic_ot_goddard()
    plotter = TrajectoryPlotter([result], fig_prefix="orbit_transfer_goddard", dim=2)
    plotter.plot_orbit_traj([(Orbit.LEO.R, "LEO"), (Orbit.HEO.R, "HEO")])
    plotter.plot_all()
