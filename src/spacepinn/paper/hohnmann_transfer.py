...
MAIN_FIGSIZE = (6.4, 6.4)
LOSS_FIGSIZE = MAIN_FIGSIZE
ORBIT_RADII = [(R_LEO, "LEO"), (R_HEO, "HEO")]
...
def plot_loss_figure(entries: list[dict], *, output_dir: str) -> None:
    ...
    ax.set_ylabel("Loss")
    ax.set_yscale("log")
    ax.legend(loc="upper right", bbox_to_anchor=(0.98, 0.98), borderaxespad=0.0)
    fig.tight_layout()
...
def plot_orbit_figure(entries: list[dict], *, output_dir: str) -> None:
    ...
    ax.scatter(0, 0, color="green", marker="o", s=EARTH_MARKER_SIZE, label="Earth")
    ax.set_xlabel("x / km")
    ax.set_ylabel("y / km")
    ax.set_aspect("equal")
    ax.legend(loc="lower right", framealpha=0.95, facecolor="white", edgecolor="0.3")
    fig.tight_layout()
