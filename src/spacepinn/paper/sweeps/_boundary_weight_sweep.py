...
def _plot_results(
    ...
    *,
    ...
    ax.scatter(lambda_values, min_total_loss, color="#1f77b4", s=18, label="Smallest total loss")
    if np.isfinite(min_bc_loss).any():
        ax.scatter(lambda_values, min_bc_loss, color="#ff7f0e", s=18, label="Smallest BC loss")
    ...
    if best is not None:
        ...
        if best_y is not None:
            ax.scatter(
                ...
                color="red",
                s=60,
                label=rf"$\lambda_{{BC}} = {float(best['lambda_bc']):.4g}$",
            )

    ax.set_xscale("log")
    ax.set_yscale("log")
    ax.set_xlabel(r"$\lambda_{BC}$")
