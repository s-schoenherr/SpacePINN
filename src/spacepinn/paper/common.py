...
def resolve_run_dir(run_dir: str | Path = DEFAULT_RUN_PLACEHOLDER) -> Path:
    ...
    if "<run_id>" in str(run_path):
        raise ValueError(
            "Set run_dir to a concrete run folder instead of the <run_id> placeholder before executing the example, "
            'for example Path("runs") / "2026" / "03" / "20260306_204631_circular_ot_transfer_learning".'
        )
