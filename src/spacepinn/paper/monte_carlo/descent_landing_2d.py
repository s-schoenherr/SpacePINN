...
from spacepinn.paper.descent_landing_2d import (
    BASELINE_COLOR,
    BASELINE_LABEL,
    ...
    PINN_COLOR,
    PINN_LABEL,
    _apply_paper_entry_styles,
    build_config as build_single_config,
    plot_altitude_figure,
    plot_gravity_figure,
    plot_orbit_figure,
    plot_polar_thrust_figure,
    plot_thrust_figure,
)
...
COLLECTION_LABEL = "descent_landing_2d_monte_carlo"
FIG_PREFIX = "descent_landing_2d_monte_carlo"
NUM_SEEDS = 100
SEEDS = [11000 + index for index in range(NUM_SEEDS)]
SMOKE_NUM_SEEDS = 2
BOXPLOT_FIGSIZE = (17.2, 4.8)
COLORS = {PINN_LABEL: PINN_COLOR}
BASELINE_LABELS = (BASELINE_LABEL,)
DEFAULT_BASELINE_RUN = RUN_ROOT / "2026" / "04" / "20260420_144325_descent_landing_2d"
MONTE_CARLO_CONVERGENCE_THRESHOLD = PAPER_CONVERGENCE_THRESHOLD
DEFAULT_ALPHA_FINAL_PI = 0.5
DEFAULT_FIXED_FINAL_ANGLE = True
DEFAULT_ATMOSPHERE = True


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Paper Monte Carlo for 2D descent landing.")
    parser.add_argument("--alpha-final-pi", type=float, default=DEFAULT_ALPHA_FINAL_PI)
    parser.add_argument("--time-guess-scale", type=float, default=1.0)
    parser.add_argument(
        "--fixed-final-angle",
        action=argparse.BooleanOptionalAction,
        default=DEFAULT_FIXED_FINAL_ANGLE,
    )
    parser.add_argument(
        "--atmosphere",
        action=argparse.BooleanOptionalAction,
...
def _build_seed_config(
    seed: int,
    *,
    alpha_final_pi: float = DEFAULT_ALPHA_FINAL_PI,
    time_guess_scale: float = 1.0,
    fixed_final_angle: bool = DEFAULT_FIXED_FINAL_ANGLE,
    atmosphere: bool = DEFAULT_ATMOSPHERE,
...
def _representative_entries(collection_run: dict, *, baseline_run: str | Path) -> list[dict]:
    ...
    best_entry = select_best_entry(grouped_entries[PINN_LABEL])
    baselines = load_reused_baseline_entries(baseline_run)
    return _apply_paper_entry_styles([{**best_entry, "label": PINN_LABEL}] + baselines)
...
def plot_collection_run(collection_run: dict, *, baseline_run: str | Path, output_dir: str | Path | None = None) -> dict[str, list[dict]]:
    ...
    print_monte_carlo_summary(grouped_entries, title="2D descent landing Monte Carlo")
    print_baseline_delta_v_summary(
        summary_entries,
    ...
    plot_polar_thrust_figure(representative_entries, output_dir=str(target_dir))
    plot_gravity_figure(representative_entries, output_dir=str(target_dir))
    plot_altitude_figure(representative_entries, output_dir=str(target_dir))
    ...
    try:
        with mp.get_context("spawn").Pool(processes=workers) as pool:
            completed_entries = pool.starmap(
                _run_seed,
                [
                    (seed, alpha_final_pi, time_guess_scale, fixed_final_angle, atmosphere, bool(smoke))
                    for seed in seeds
                ],
            )
