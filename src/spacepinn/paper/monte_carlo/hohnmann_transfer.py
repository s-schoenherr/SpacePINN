...
from spacepinn.paper.hohnmann_transfer import (
    BASELINE_LABEL,
    DIRECT_COLLOCATION_COLOR,
    KINEMATIC_LABEL,
    MAIN_FIGSIZE,
...
COLLECTION_LABEL = "hohnmann_transfer_monte_carlo"
FIG_PREFIX = "hohnmann_transfer_monte_carlo"
NUM_SEEDS = 100
SEEDS = [4000 + index for index in range(NUM_SEEDS)]
SMOKE_NUM_SEEDS = 2
BOXPLOT_FIGSIZE = (17.2, 4.8)
COLORS = {
    KINEMATIC_LABEL: PALETTE["kinematic"],
}
MONTE_CARLO_CONVERGENCE_THRESHOLD = 1e-6


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Paper Monte Carlo for Hohnmann transfer.")
    parser.add_argument(
        "--reuse-baseline-from",
        default=None,
        help="Optional saved run directory to copy the baseline entry from instead of recomputing it.",
    )
    parser.add_argument(
        "--from-run",
        default=None,
...
def build_configs(*, smoke: bool | None = None) -> list[dict]:
    return [_build_seed_config(seed, smoke=smoke) for seed in get_seeds(smoke=smoke)]


def _build_seed_config(seed: int, *, smoke: bool | None = None) -> dict:
    config = deepcopy(build_single_config(smoke=smoke))
    config["label"] = f"{KINEMATIC_LABEL} | seed={seed}"
    config["seed"] = seed
...
def group_entries(entries: list[dict]) -> dict[str, list[dict]]:
    grouped = {KINEMATIC_LABEL: []}
    for entry in entries:
        label = str(entry.get("label", ""))
        if label.startswith(KINEMATIC_LABEL):
            grouped[KINEMATIC_LABEL].append(entry)
    return grouped
...
def _normalize_baseline_entry_style(entry: dict) -> dict:
    ...
    plotting["color"] = DIRECT_COLLOCATION_COLOR
    ...
    normalized["color"] = DIRECT_COLLOCATION_COLOR
...
def load_reused_baseline_entry(run_dir: str | Path) -> dict:
    loaded_run = load_run(run_dir)
    baseline_entry = get_baseline_entry(loaded_run)
    reused_entry = {
    ...
    return _normalize_baseline_entry_style(reused_entry)
...
def _run_seed(seed: int, smoke: bool = False) -> dict:
    config = _build_seed_config(seed, smoke=smoke)
...
def _representative_entries(collection_run: dict) -> list[dict]:
    ...
    best_entry = select_best_entry(grouped_entries[KINEMATIC_LABEL])
    baseline_entry = get_baseline_entry(collection_run)
    return [
        {
            **best_entry,
            "label": KINEMATIC_LABEL,
        },
        {
            **baseline_entry,
            "plotting": {
                **baseline_entry.get("plotting", {}),
                "color": baseline_entry.get("plotting", {}).get("color", DIRECT_COLLOCATION_COLOR),
            },
        },
    ]
...
def plot_collection_run(collection_run: dict, *, output_dir: str | Path | None = None) -> dict[str, list[dict]]:
    ...
    print_monte_carlo_summary(grouped_entries, title="Hohnmann transfer Monte Carlo")
    print_baseline_delta_v_summary(
        summary_entries,
    ...
    plot_orbit_figure(representative_entries, output_dir=str(target_dir))
...
def _finalize_collection_entries(
    ...
    *,
    ...
    reused_baseline_entry: dict | None = None,
) -> dict:
    return run_experiment_collection(
        ...
        + [
            reused_baseline_entry
            if reused_baseline_entry is not None
...
def run_collection(
    *,
    smoke: bool | None = None,
    workers: int = 1,
    reuse_baseline_from: str | Path | None = None,
) -> dict:
    seeds = get_seeds(smoke=smoke)
    reused_baseline_entry = load_reused_baseline_entry(reuse_baseline_from) if reuse_baseline_from is not None else None

    if workers <= 1:
        return _finalize_collection_entries(
            [_run_seed(seed, smoke=bool(smoke)) for seed in seeds],
            smoke=smoke,
            reused_baseline_entry=reused_baseline_entry,
        )
    ...
    try:
        with mp.get_context("spawn").Pool(processes=workers) as pool:
            completed_entries = pool.starmap(_run_seed, [(seed, bool(smoke)) for seed in seeds])
...
def main(
    *,
    ...
    collection_run = run_collection(smoke=smoke, workers=workers, reuse_baseline_from=reuse_baseline_from)
    persist_paper_monte_carlo_aggregate_summary(
        collection_run,
        title=COLLECTION_LABEL,
        baseline_labels=(BASELINE_LABEL,),
