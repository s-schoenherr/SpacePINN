...
def _build_ordinary_config(seed: int, *, smoke: bool | None = None) -> dict:
    config = deepcopy(soft_bc_3d_config)
...
def group_entries(entries: list[dict]) -> dict[str, list[dict]]:
    ...
    for entry in entries:
        ...
        if label.startswith(PRETRAINED_LABEL):
            grouped[PRETRAINED_LABEL].append(entry)
        elif label.startswith(GEOMETRIC_LABEL):
...
def plot_monte_carlo_boxplots_paper(
    ...
    *,
    ...
    display_labels = [
        r"$\mathrm{Exact\ BC}$",
        r"$\mathrm{Soft\ BC}$",
        r"$\mathrm{Exact\ BC + pre\text{-}cond.}$",
    ]
...
def run_collection(*, smoke: bool | None = None, workers: int = 1) -> dict:
    ...
    if workers <= 1:
        ...
        return _finalize_collection_entries(entries)
...
def _finalize_collection_entries(entries: list[dict]) -> dict:
