...
FIG_PREFIX = "weights2d_dynamic"
DATA_DIR = Path(spacepinn.__file__).resolve().parents[2] / "data" / "runs" / "appendix" / "boundary_weight_search_2d"
...
def replot_saved_run(
    ...
    *,
    ...
    if print_summary:
        ...
        print(
            f"[boundary_weight_search_2d_dynamic] wrote {output_path} "
    ...
    *,
    ...
    if run_sweep:
        return run_boundary_weight_sweep(
            spec=SweepSpec(
                label="boundary_weight_search_2d_dynamic",
                dimension=2,
                dynamic_tof=True,
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Boundary loss weight search (2D).")
